...
include('DataClass2.php');
include('DataClass3.php');
...
function getdata2()
{
	...
	$dataobject2 = new TrafficData2();
	$dataobject2->getwazealerts($zipcode,"POLICE");
	...
	$wazelatitude = $dataobject2->getwazearray("LAT");
	$wazelongitude = $dataobject2->getwazearray("LONG");
	$thumbsup = $dataobject2->getwazearray("THUMBS");
	$wazesubtype = $dataobject2->getwazearray("TYPE");
...
function getdata3()
{
	...
	$dataobject3 = new TrafficData3();

	$accidentlatitude= array();
	$accidentlongitude = array();
	$accidentthumbsup = array();
	$accidentwazesubtype = array();

	$dataobject3->getwazealerts($zipcode,"ACCIDENT");

	$accidentlatitude = $dataobject3->getwazearray("LAT");
	$accidentlongitude = $dataobject3->getwazearray("LONG");
	$accidentthumbsup = $dataobject3->getwazearray("THUMBS");
	$accidentwazesubtype = $dataobject3->getwazearray("TYPE");
...
<body>
  ...
  <div class="container">
    ...
    <div id="home" class="tab-pane fade in active">
	  ...
	  var wazelatitudearray = <?php echo json_encode($wazelatitude) ?>;
	  var wazelongitudearray = <?php echo json_encode($wazelongitude) ?>;
	  var thumbsupearray = <?php echo json_encode($thumbsup) ?>;
	  var typearray = <?php echo json_encode($wazesubtype) ?>;
	  ...
	  var wazestartcoordinates = [];

	  var accidentstartcoordinates = [];
	  ...
	  for (var i = 0; i < wazelongitudearray.length; i++)
	  {
		  wazestartcoordinates.push({});
	  }

	  for( var j = 0; j < wazelongitudearray.length; j++)
	  {
	  	wazestartcoordinates[j].lat = wazelatitudearray[j];
	  	wazestartcoordinates[j].lng = wazelongitudearray[j];
	  }

	  var wazeincidentmarkers = [];
		...
		function drawmarkers(map)
		 ...
		 function drawconsmarkers(map)
		  {  
			  ...
			  for(var j = 0; j< wazestartcoordinates.length ; j++)
			  {
					
				consmarkernew = new google.maps.Marker({
				    map: map,
					position: new google.maps.LatLng(wazestartcoordinates[j].lat,wazestartcoordinates[j].lng),
					title: thumbsupearray[j],
					icon: 'police.png',
					animation: google.maps.Animation.DROP
	   			});

				var content = thumbsupearray[j];   
               var infowindow = new google.maps.InfoWindow();
				...
				wazeincidentmarkers.push(consmarkernew);
	...
	&copy 2015 Group 7
		    ...
		    <form action="Police.php" method = "post" role="form">
