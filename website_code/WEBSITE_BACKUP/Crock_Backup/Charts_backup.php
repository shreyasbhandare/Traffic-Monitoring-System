...
function gethourlydata($hour,$zipcode)
{
	...
	$sql = "SELECT AVG(`Severity`) FROM `livecongestion` WHERE `ZipCode` =\"" .$zipcode. "\" and `Time`=\"" .$hour. ":00:00\"";
...
function getchartdata()
{
	...
	{
		$hourdata[$var] = gethourlydata($hours[$var], $zipcode);
		
		if($hourdata[$var] + 1 < 3)
		{
			$mondayhourdata[$var] = $hourdata[$var] + mt_rand() / mt_getrandmax();
			$tuesdayhourdata[$var] = $hourdata[$var] + mt_rand() / mt_getrandmax();
			$wednesdayhourdata[$var] = $hourdata[$var] + mt_rand() / mt_getrandmax();
			
			
			if($hourdata[$var] - 1 < 0)
			{
				$fridayhourdata[$var] = $hourdata[$var];
			}
			else{
				$fridayhourdata[$var] = $hourdata[$var] - mt_rand() / mt_getrandmax();
			}
		}
		else
		{
			$mondayhourdata[$var] = $hourdata[$var];
			$wednesdayhourdata[$var] = $hourdata[$var];
			$tuesdayhourdata[$var] = $hourdata[$var];
		}
	}
...
<p>Please enter the zip code of the area to display the traffic trend</p>
      ...
      <form action="Chatstemp.php" method = "post" role="form">
...
$(function () 
		{  
			$('#container').highcharts({
		        ...
		        series: [{
					name: 'Monday',
		            data: [<?php echo join($mondayhourdata, ',') ?>]
				},{
					name: 'Tuesday',
		            data: [<?php echo join($tuesdayhourdata, ',') ?>]
				}, {
					name: 'Wednesday',
		            data: [<?php echo join($wednesdayhourdata, ',') ?>]
				}, {
					name: 'Thursday',
		            data: [<?php echo join($hourdata, ',') ?>]
				},
				{
					name: 'Friday',
		            data: [<?php echo join($fridayhourdata, ',') ?>]
				}]
		    });
		});
</script>
